...
<table lay-filter="tabledata">
    <thead>
    <tr>
        <th lay-data="{escape:false,align:'center',field:'a' }">标题</th>
        <th lay-data="{escape:false,align:'center',field:'b' }">分类</th>
        <th lay-data="{escape:false,align:'center',field:'c' }">是否置顶</th>
        <th lay-data="{escape:false,align:'center',field:'d' }">是否轮播</th>
        <th lay-data="{escape:false,align:'center',field:'e' }">发布日期</th>
        <th lay-data="{escape:false,align:'center',field:'f' }">点击数</th>
    ...
    <c:forEach items="${broadcastList}" var="broadcast">
        <tr>
            <td align="center">${broadcast.title}</td>
            <td align="center">${broadcast.broadcast_kindsname}</td>
            <td align="center">${broadcast.istop}</td>
            <td align="center">${broadcast.isflv}</td>
            <td align="center">
                <fmt:formatDate value="${broadcast.addtime}" pattern="yyyy年MM月dd日"/>
            </td>
            <td align="center">${broadcast.hits}</td>

            <td align="center">
                <a href="manager/broadcast/getById.do?id=${broadcast.broadcastid}"   class="layui-btn layui-btn-xs" >编辑</a>
                <a href="manager/broadcast/delete.do?id=${broadcast.broadcastid}"
    ...
    <TR align="right">
        <TD>
            <form class="list_page" action="" method="get" name="formpage">
                ...
                <!--//用于给上面javascript传值-->
                <input type="hidden" name="jumpurl" value="manager/broadcast/getAll.do?"/>
                <!--//用于给上面javascript传值-->
