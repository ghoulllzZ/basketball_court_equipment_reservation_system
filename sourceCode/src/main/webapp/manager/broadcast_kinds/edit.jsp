...
<div class="layui-row">
    ...
    <form class="layui-form layui_form_web" method="post" action="manager/broadcast_kinds/update.do"
          name="myform" onsubmit="return check()">
        <input
                type="hidden" name="broadcast_kindsid" value="${broadcast_kinds.broadcast_kindsid}"/>

        <div class="layui-form-item">
            <label class="layui-form-label">分类名称</label>
            <div class="layui-input-block"><input type="text" lay-verify="required" required="required"
                                                  class="layui-input" name="broadcast_kindsname"
                                                  style="width: 160px" id="broadcast_kindsname"
                                                  value="${broadcast_kinds.broadcast_kindsname}"/></div>
        </div>
        <div class="layui-form-item">
...
<script>
    function check() {
        if (document.myform.broadcast_kindsname.value == '') {
            alert('请输入分类名称');
            return false;
        }
    }
