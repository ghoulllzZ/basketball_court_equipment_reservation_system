...
<table lay-filter="tabledata">
    ...
    <tr>
        ...
        <th lay-data="{escape:false,align:'center',field:'c' }">联系方式</th>
        <th lay-data="{escape:false,align:'center',field:'d' }">操作</th>
    </tr>
    </thead>
    <tbody>
    <c:forEach items="${managerList}" var="manager">
        <tr>
            ...
            <td align="center">${manager.realname}</td>
            <td align="center">${manager.contact}</td>
            <td align="center">
                <c:if test="${sessionScope.managerid eq manager.managerid ||sessionScope.managerid eq '1'  }">

                    <a href="manager/manager/getById.do?id=${manager.managerid}"
                       class="layui-btn layui-btn-xs">编辑</a>
                </c:if>
                <c:if test="${sessionScope.managerid eq '1' }">
                    <a href="manager/manager/delete.do?id=${manager.managerid}"
    ...
    <TR align="right">
        <TD>
            <form class="list_page" action="" method="get" name="formpage">
                ...
                <!--//用于给上面javascript传值-->
                <input type="hidden" name="jumpurl" value="manager/manager/getAll.do?"/>
                <!--//用于给上面javascript传值-->
