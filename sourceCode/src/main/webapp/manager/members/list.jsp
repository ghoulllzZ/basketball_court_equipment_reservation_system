...
<table lay-filter="tabledata">
    <thead>
    <tr>
        <th lay-data="{escape:false,align:'center',field:'a1' }">照片</th>
        <th lay-data="{escape:false,align:'center',field:'a' }">用户名</th>
        <th lay-data="{escape:false,align:'center',field:'b' }">姓名</th>

        <th lay-data="{escape:false,align:'center',field:'c' }">性别</th>
        <th lay-data="{escape:false,align:'center',field:'d' }">出生日期</th>
        <th lay-data="{escape:false,align:'center',field:'e' }">联系方式</th>
        <th lay-data="{escape:false,align:'center',field:'f' }">注册日期</th>
        <th lay-data="{escape:false,align:'center',field:'f1' }">联系地址</th>
        <th lay-data="{escape:false,align:'center',field:'g1' }">状态</th>
        <th lay-data="{escape:false,align:'center',field:'g',width:200 }">操作</th>
    </tr>
    </thead>
    <tbody>
    <c:forEach items="${membersList}" var="members">
        <tr>
            <td align="center"><img src="${members.image}" width="30"></td>
            <td align="center">${members.username}</td>
            <td align="center">${members.realname}</td>

            <td align="center">${members.sex}</td>
            <td align="center">${members.birthday}</td>
            <td align="center">${members.contact}</td>

            <td align="center">
                <fmt:formatDate value="${members.regdate}" pattern="yyyy年MM月dd日"/>
            </td>
            <td align="center">${members.address}</td>
            <td align="center">
                <c:if test="${members.iscooled==0}">
                    正常
                </c:if>
                <c:if test="${members.iscooled==1}">
                    冻结
                </c:if>
            ...
            <td align="center">
                <a
                        href="manager/members/iscooled.do?iscooled=1&id=${members.membersid}"
                        class="layui-btn layui-btn-danger layui-btn-xs">冻结</a>
                <a
                        href="manager/members/iscooled.do?iscooled=0&id=${members.membersid}"
                        class="layui-btn layui-btn-xs">正常</a>
                <a
                        href="manager/members/delete.do?id=${members.membersid}"
    ...
    <TR align="right">
        <TD>
            <form class="list_page" action="" method="get" name="formpage">
                ...
                <!--//用于给上面javascript传值-->
                <input type="hidden" name="jumpurl" value="manager/members/getAll.do?"/>
                <!--//用于给上面javascript传值-->
